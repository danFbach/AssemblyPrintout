...
namespace AssemblyPrintout
{
    class Menu
    {
                ...
                switch (args[0])
                {
                    ...
                    case "-bkod":
                        ...
                        if (args[1].Length == 8)
                        {
                            ...
                            else Utilities.ExceptionExit("Date format was invalid and not parseable.", null, true);
                        }
                        Environment.Exit(0);
                        break;
                    case "-bko":
                        ...
                        break;
                    case "-a":
                        if (File.Exists(Paths.ReadExportData))
                        {
                            ///GETS DATA FOR NEW ASSEMBLY SCHEDULE AND DAILY_7 PRINTOUT
                            List<string> data0 = Read.ExportReader(Paths.ReadExportData);
                    ...
                    case "-p":
                        ...
                        break;
                    case "-q":
                        Write.QuickSortWriter(AssemblyParser.Quicksort(Read.GenericRead(Paths.ReadExportData).ToList()), Paths.WriteExportData);
                        Process.Start("notepad.exe", Paths.WriteExportData);
                        Environment.Exit(0);
                        break;
                }
            }
        ...
        {
            List<string> data1 = Read.GenericRead(Paths.Production).ToList(); ///GETS THIS MONTHS PRODUCTION DATA
            if (data1.Any() && Utilities.Products.Any()) ///IF WE HAVE PRODUCTION DATA AND ASSMEBLY TIMES, PROCEED
            {
                ...
                string[] TempCurrentProductionData = Read.GenericRead(Paths.InitPath).ToArray();
                ...
                using (StreamWriter sw = new StreamWriter(Paths.InitPath))
