...
namespace AssemblyPrintout
{
    class Write
    {
        ...
        public void Writer(datatypes.datasetRAW dsr, string path)
        {
            path += ".txt";
            ...
            try
            {
                using (StreamWriter sw = File.CreateText(path))
                {
                    ...
                    sw.WriteLine(top);
                    foreach (datatypes.pcode p in dsr.pcodes)
                    {
                        ...
                        {
                            foreach (datatypes.product _p in p.productList)
                            {
        ...
        {
            try { using (StreamWriter sw = new StreamWriter(@"C:\INVEN\Error.txt")) { sw.WriteLine(error); } }
            catch (Exception e) { Environment.Exit(0); }
        }
    }
}
